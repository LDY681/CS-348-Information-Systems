...
query = (	
"select u.occupation as useroccu, COUNT(u.userid) as usercnt from users as u group by u.occupation"
)
results = sqlContext.sql(query)

# write data back to HDFS;
# repartition down to 10 partitions to reduce number of HDFS files written;
results.rdd.map(lambda p: str(p.useroccu)+"::"+str(p.usercnt)).repartition(10).saveAsTextFile(sys.argv[2])
