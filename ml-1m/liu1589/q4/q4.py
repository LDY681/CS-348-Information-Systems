...
query = (	
"select m.title as title, ROUND(AVG(r.rating),1) as avg from movies as m join ratings as r on m.movieid = r.movieid where SUBSTRING_INDEX(m.title, '(', -1) = '1998)' group by m.title"
)
results = sqlContext.sql(query)

# write data back to HDFS;
# repartition down to 10 partitions to reduce number of HDFS files written;
results.rdd.map(lambda p: str(p.title)+"::"+str(p.avg)).repartition(10).saveAsTextFile(sys.argv[2])
